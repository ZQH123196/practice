...
DROP DATABASE IF EXISTS `rbac0WithDataScope`;
CREATE DATABASE  `rbac0WithDataScope` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
SET NAMES utf8mb4;
...
CREATE TABLE `user` (
    ...
    create_time datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    UNIQUE INDEX `idx_user` (`username` ASC) USING BTREE,
    PRIMARY KEY (id)
...
CREATE TABLE `user_group_hierarchy` (
    `user_group_id`  int(11) NOT NULL COMMENT '用户组的编号，相当于 node 的 value',
    `node_name`  varchar(11)  NOT NULL COMMENT '一般与 user_group_id 相同',
    `path_enum`  varchar(100)  NOT NULL COMMENT '以 , 分割的从根 node_name 到当前 node_name 的枚举路径，别留空格',
